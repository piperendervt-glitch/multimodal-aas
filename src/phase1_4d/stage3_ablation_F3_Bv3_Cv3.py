"""Phase 1.4d Stage 3 Ablation F-3 — 2-topology subset (B_v3 + C_v3).

Removes the pure-audio topology (A) from Target C's ensemble and re-runs
the weight + consensus-gate + multiplicative-integration pipeline on the
remaining pair (pure visual B_v3 + temporal-sync fusion C_v3). Paired
against F-1 (A + B_v3) and F-2 (A + C_v3), this isolates the contribution
of Topology A (BirdNET → LLM pure-audio pipeline).
"""
...
OUT_DIR = REPO_ROOT / "results" / "phase1_4d" / "stage3_ablation_F3"
...
SUBSET = ("B", "C")  # B = Topology B_v3, C = Topology C_v3 — excludes A (pure audio)
SUBSET_LABEL = "B_v3 + C_v3 (pure visual + temporal-sync fusion, pure-audio excluded)"
REMOVED = "A (pure-audio, BirdNET → LLM)"
...
def render_summary(
    ...
    lines = render_common_summary_head(
        title=f"Phase 1.4d Stage 3 Ablation F-3 — 2-topology subset ({SUBSET_LABEL})",
...
def main() -> int:
    ...
    print()
    print("=== Phase 1.4d Stage 3 Ablation F-3 summary ===")
    print(

"""Phase 1.4d Stage 3 Ablation F-2 — 2-topology subset (A + C_v3).

Removes the pure-visual topology (B_v3) from Target C's ensemble and
re-runs the weight + consensus-gate + multiplicative-integration pipeline
on the remaining pair (pure audio A + temporal-sync fusion C_v3).
Paired against F-1 (A + B_v3) and F-3 (B_v3 + C_v3), this isolates the
contribution of the pure-visual topology.
"""
...
OUT_DIR = REPO_ROOT / "results" / "phase1_4d" / "stage3_ablation_F2"
...
SUBSET = ("A", "C")  # A = Topology A, C = Topology C_v3 — excludes B_v3 (pure visual)
SUBSET_LABEL = "A + C_v3 (pure audio + temporal-sync fusion, pure-visual excluded)"
REMOVED = "B_v3 (pure-visual, bbox v3 prompt)"
...
def render_summary(
    ...
    lines = render_common_summary_head(
        title=f"Phase 1.4d Stage 3 Ablation F-2 — 2-topology subset ({SUBSET_LABEL})",
...
def main() -> int:
    ...
    print()
    print("=== Phase 1.4d Stage 3 Ablation F-2 summary ===")
    print(
